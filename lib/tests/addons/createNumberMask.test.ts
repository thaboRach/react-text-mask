...
describe('createNumberMask', () => {
  ...
  it('can be configured to add a thousands separator or not', () => {
    let numberMaskWithoutThousandsSeparator = createNumberMask({
      includeThousandsSeparator: false,
    });
    expect(numberMaskWithoutThousandsSeparator('1000')).to.deep.equal([
      '$',
      /\d/,
      /\d/,
      /\d/,
    ...
    expect(numberMaskWithThousandsSeparator('1000')).to.deep.equal([
      '$',
      /\d/,
      ',',
      /\d/,
  ...
  it('can limit the length of the fraction', () => {
    ...
    expect(numberMask('1000.3823')).to.deep.equal([
      '$',
      /\d/,
      ',',
      /\d/,
...
describe('integer limiting', () => {
  ...
  it('works when there is a thousands separator', () => {
    expect(createNumberMask({ integerLimit: 4, prefix: '' })('1,9995')).to.deep.equal([
      /\d/,
      ',',
      /\d/,
      /\d/,
    ...
    expect(createNumberMask({ integerLimit: 7, prefix: '' })('1,000,0001')).to.deep.equal([
      /\d/,
      ',',
      /\d/,
      /\d/,
  ...
  it('works when there is a decimal and a prefix', () => {
    ...
    expect(numberMask('$199.34')).to.deep.equal([
      '$',
      /\d/,
      /\d/,
      /\d/,
  ...
  it('works when thousandsSeparatorSymbol is a period', () => {
    ...
    expect(numberMask('1234567890,12345678')).to.deep.equal([
      /\d/,
      /\d/,
      '.',
      /\d/,
...
describe('numberMask default behavior', () => {
  ...
  it('adds as many commas as needed', () => {
    const numberMask = createNumberMask({});
    expect(numberMask('23984209342084')).to.deep.equal([
      '$',
      /\d/,
      /\d/,
      ',',
